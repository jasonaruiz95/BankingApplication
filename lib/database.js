...
function createTables() {
    ...
    sql = "CREATE PROCEDURE IF NOT EXISTS `get_top_ten_transactions`(\n" +
                  ...
                  "BEGIN\n" +
                    ...
                    "WHERE t.account_id = \n" +
                    "(SELECT account_id FROM accounts INNER JOIN users ON accounts.user_id = users.user_id WHERE users.username = username)\n" +
                    "AND accounts.account_name = account_name\n" +
                    "ORDER BY t.date ASC\n" +
                    "LIMIT 10;\n" +
              "END;";
