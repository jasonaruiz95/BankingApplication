...
function createTables() {
    ...
    sql = "CREATE PROCEDURE IF NOT EXISTS `insert_balance`(\n" +
                     "IN account_id INT,\n" +    
                     "IN balance INT\n" + 
                  ")\n" +
                  "BEGIN\n" +
    ...
    con.query(sql, function(err, results, fields) {
      if (err) {
        ...
        console.log("database.js: procedure insert_balance created if it didn't exist");
      }
    });
    ...
    sql = "CREATE PROCEDURE IF NOT EXISTS `get_top_ten_transfers`(\n" +
    ...
    function addDummyDataToDatabase() {
        ...
        // sql = "CALL insert_user('jason', 'ruiz', 'smelly@aol.com', 'avada kedavra', @result)";
        // con.query(sql, function(err,rows){
        //   if (err) {
        //     console.log(err.message);
        //     throw err;
